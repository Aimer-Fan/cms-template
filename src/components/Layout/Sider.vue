...
function generateSubMenuTitle (router: RouteRecordRaw, t: Composer['t']) {
  return (
    <span>
      { generateIcon(router.meta?.icon) }
      <span>{t(router.meta?.title || '')}</span>
    </span>
  )
}

function generateMenu (routerList: Array<RouteRecordRaw>, t: Composer['t'], basePath?: string) {
  return routerList.map(router => {
    const resolved = resolvePath(basePath || '', router.path)
    if (router.children && router.children.length) {
      return (
        <Menu.SubMenu title={generateSubMenuTitle(router, t)} key={resolved}>
          { generateMenu(router.children, t, resolved) }
        </Menu.SubMenu>
      )
    } else {
      return (
        <Menu.Item title={t(router.meta?.title || '')} key={resolvePath(basePath || '', router.path)}>
...
export default defineComponent({
  ...
  setup () {
    ...
    return () => (
      ...
      >
        {generateMenu(menuRouters.value || [], t)}
      </Menu>
    )
  }
})
